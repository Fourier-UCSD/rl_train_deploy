...
class AMPLoader:
    ...
    def __init__(
            ...
            motion_files=glob.glob('datasets/motion_train/*'),
            ):
    ...
    def slerp(self, frame1, frame2, blend):
        slerped_q_pevils = self.slerp_q(frame1[:,:4], frame2[:,:4], blend)
        slerped_q_leftthigh = self.slerp_q(frame1[:,4:8], frame2[:,4:8], blend)
        slerped_q_leftshin = self.slerp_q(frame1[:,8:12], frame2[:,8:12], blend)
        slerped_q_leftfoot = self.slerp_q(frame1[:,12:16], frame2[:,12:16], blend)
        slerped_q_rightthigh = self.slerp_q(frame1[:,16:20], frame2[:,16:20], blend)
        slerped_q_rightshin = self.slerp_q(frame1[:,20:24], frame2[:,20:24], blend)
        slerped_q_rightfoot = self.slerp_q(frame1[:,24:28], frame2[:,24:28], blend)
        slerped_v = self.slerp_v(frame1[:,28:], frame2[:,28:], blend)

        return torch.cat((slerped_q_pevils,
                            slerped_q_leftthigh,slerped_q_leftshin, slerped_q_leftfoot,
                            slerped_q_rightthigh,slerped_q_rightshin,slerped_q_rightfoot,
                            slerped_v), dim=-1)

    def slerp_v(self, val0, val1, blend):
        return (1.0 - blend) * val0 + blend * val1

    def slerp_q(self,q0, q1, t):
    ...
    def blend_frame_pose(self, frame0, frame1, blend):
        ...
        blend_q_pevils = self.slerp_q(joints0[:4], joints1[:4], blend)
        blend_q_leftthigh = self.slerp_q(joints0[4:8], joints1[4:8], blend)
        blend_q_leftshin = self.slerp_q(joints0[8:12], joints1[8:12], blend)
        blend_q_leftfoot = self.slerp_q(joints0[12:16], joints1[12:16], blend)
        blend_q_rightthigh = self.slerp_q(joints0[16:20], joints1[16:20], blend)
        blend_q_rightshin = self.slerp_q(joints0[20:24], joints1[20:24], blend)
        blend_q_rightfoot = self.slerp_q(joints0[24:28], joints1[24:28], blend)
        ...
        blend_joints_vel = self.slerp_v(joint_vel_0, joint_vel_1, blend)

        return torch.cat([
            blend_q_pevils,
            blend_q_leftthigh, blend_q_leftshin,blend_q_leftfoot,
            blend_q_rightthigh,blend_q_rightshin,blend_q_rightfoot,
            blend_joints_vel])
    ...
    def observation_dim(self):
        ...
        # print(f"self.trajectories[0].shape[1]{self.trajectories[0].shape[1]}")
